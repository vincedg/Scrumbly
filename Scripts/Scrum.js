...
function drawNewCard(id, text, x, y, rot, colour, sticker, animationspeed) {
    ...
    var h = '<div id="' + id + '" class="card ' + colour +
        ...
        'deg);\
	">\
	<img src="/Content/images/icons/token/Xion.png" class="card-icon delete-card-icon" />\
	<img class="card-image" src="/Content/images/' +
        colour + '-card.png">\
	<div id="content:' + id +
...
function addSticker(cardId, stickerId) {
    ...
    if (Array.isArray(stickerId)) {
        for (var i in stickerId) {
            stickerContainer.prepend('<img src="/Content/images/stickers/' + stickerId[i] +
                '.png">');
        }
    } else {
        if (stickerContainer.html().indexOf(stickerId) < 0)
            stickerContainer.prepend('<img src="/Content/images/stickers/' + stickerId +
                '.png">');
    }
...
function drawNewColumn(columnName) {
    ...
    $('.editable').editable(function (value, settings) {
        ...
        width: '',
        height: '',
        xindicator: '<img src="/Content/images/ajax-loader.gif">',
        event: 'dblclick', //event: 'mouseover'
    });
...
function changeThemeTo(theme) {
    currentTheme = theme;
    $("link[title=cardsize]").attr("href", "/Content/css/" + theme + ".css");
}
